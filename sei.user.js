...
function analisarPagina() {
  ...
  return pipe(
    infoTabelas,
    map(infoTabelas => ({ divRecebidos, tabelas: infoTabelas })),
  );
}
function analisarTabela(tabela) {
  return pipe(
    tryCatch(
      () => {
        if (!hasLength(tabela.tBodies, 1)) throw new ParseError('Erro ao analisar tabela.');
        ...
        return pipe(
          all(linhasProcessos.map(analisarLinha)),
          map(processos => ({
            tabela,
            tbody,
            cabecalho,
...
function analisarTooltipLinkComMouseover(link) {
  ...
  if (!match || !hasLength(match, 3)) return err(new ParseError('Erro ao analisar tooltip.'));
...
function analisarLinha(linha, ordemOriginal) {
  ...
  const linkProcesso = cells[2].querySelector(
    'a[href^="controlador.php?acao=procedimento_trabalhar&"][onmouseover]',
  );
  if (!linkProcesso) return err(new ParseError('Link do processo não encontrado.'));
  const numeroFormatado = linkProcesso.textContent;
  if (!numeroFormatado) return err(new ParseError('Número do processo não encontrado.'));
  const numero = analisarNumeroFormatado(numeroFormatado);
  const imgAnotacao = cells[1].querySelector(
  ...
  const imgMarcador = cells[1].querySelector(
    CoresMarcadores.map(
  ...
  return pipe(
    all([analisarTooltipLinkComMouseover(linkProcesso), numero, anotacao, marcador]),
    map(([{ titulo: tipo, texto: especificacao }, numero, anotacao, marcador]) => ({
      linha,
      cells,
      link: linkProcesso,
      ordemOriginal,
...
function analisarAnotacao(imgAnotacao) {
  ...
  return pipe(
    ...
    chain(({ titulo: usuario, texto }) => {
      if (!texto) return err(new ParseError('Erro ao analisar tooltip.'));
...
function analisarMarcador(imgMarcador) {
  return pipe(
    imgMarcador.parentElement,
    analisarTooltipLinkComMouseover,
    map(({ titulo: nome, texto }) => ({
      imagem: imgMarcador,
      nome,
...
function criarSelectOrdenacao(valor, dispatch) {
  ...
  const makeValor = ({ criterio = Criterio.PADRAO, inverter = false, prioritarios = false }) =>
    sord.serialize({
      criterio,
      inverter,
      prioritarios,
...
function gerarFuncaoOrdenarPorMarcador(fnOrdenacao, inverter) {
  return altOrdering((a, b) => {
    var _a, _b, _c, _d;
    let textoA =
      (_b = (_a = a.marcador) === null || _a === void 0 ? void 0 : _a.nome.toLocaleLowerCase()) !==
        null && _b !== void 0
        ? _b
        : 'zz';
    let textoB =
      (_d = (_c = b.marcador) === null || _c === void 0 ? void 0 : _c.nome.toLocaleLowerCase()) !==
        null && _d !== void 0
        ? _d
        : 'zz';
    if (inverter) {
...
const ordenarPorAnotacao = altOrdering(
  compareUsing(x => {
    var _a, _b;
    return (_b =
      (_a = x.anotacao) === null || _a === void 0 ? void 0 : _a.texto.toLocaleLowerCase()) !==
      null && _b !== void 0
      ? _b
      : 'zz';
  }),
...
const ordenarPorAnotacaoPrioritariosPrimeiro = altOrdering(
  compareUsing(x => {
    var _a, _b;
    return (
      (_b = (_a = x.anotacao) === null || _a === void 0 ? void 0 : _a.prioridade) !== null &&
      _b !== void 0
        ? _b
        : false
    )
...
function renderizarPagina(pagina, preferencias, dispatch) {
  ...
  for (const { cabecalhoCells, processos } of pagina.tabelas) {
    ...
    for (const processo of processos) {
      ...
      processo.link.addEventListener('mouseover', () => {
        ...
        if (mostrarTooltip)
          infraTooltipMostrar(
            (_a = processo.especificacao) !== null && _a !== void 0 ? _a : '',
            processo.tipo,
          );
      });
  ...
  function atualizar(preferencias) {
    ...
    const campos = ['ocultarFieldset', 'mostrarTipo', 'mostrarAnotacoes', 'mostrarMarcadores'];
...
function main() {
  ...
  if (params.get('acao') === 'procedimento_controlar') {
    if (
      ((_a = document.querySelector('input#hdnTipoVisualizacao')) === null || _a === void 0
        ? void 0
        : _a.value) !== 'R'
    ) {
      return ok(undefined);
